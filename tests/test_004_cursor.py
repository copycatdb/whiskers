...
#     """Test setinputsizes"""
#     sizes = [(whiskers.ConstantsDDBC.SQL_INTEGER, 10), (whiskers.ConstantsDDBC.SQL_VARCHAR, 255)]
#     cursor.setinputsizes(sizes)

# def test_setoutputsize(cursor):
#     """Test setoutputsize"""
#     cursor.setoutputsize(10, whiskers.ConstantsDDBC.SQL_INTEGER)
...
def test_lowercase_setting_after_cursor_creation(cursor, db_connection):
    ...
    original_lowercase = whiskers.lowercase
    try:
        # Create table and execute with lowercase=False
        whiskers.lowercase = False
        ...
        whiskers.lowercase = original_lowercase
        try:
...
def test_rownumber_warning_logged(cursor, db_connection):
    ...
    import logging
    from whiskers.helpers import get_logger
...
def test_scroll_backward_not_supported(cursor, db_connection):
    ...
    from whiskers.exceptions import NotSupportedError
    try:
...
def test_cursor_skip_invalid_arguments(cursor, db_connection):
    ...
    from whiskers.exceptions import ProgrammingError, NotSupportedError
...
def test_decimal_separator_with_multiple_values(cursor, db_connection):
    ...
    original_separator = whiskers.getDecimalSeparator()
    ...
    try:
        ...
        whiskers.setDecimalSeparator(original_separator)
...
def test_decimal_separator_basic_functionality():
    ...
    try:
        # Test default value
        assert whiskers.getDecimalSeparator() == '.', "Default decimal separator should be '.'"
        
        # Test setting to comma
        whiskers.setDecimalSeparator(',')
        assert whiskers.getDecimalSeparator() == ',', "Decimal separator should be ',' after setting"
        ...
        # Test invalid inputs
        with pytest.raises(ValueError):
            whiskers.setDecimalSeparator('')  # Empty string
        
        with pytest.raises(ValueError):
            whiskers.setDecimalSeparator('too_long')  # More than one character
        
        with pytest.raises(ValueError):
            whiskers.setDecimalSeparator(123)  # Not a string
...
def test_cursor_setinputsizes_basic(db_connection):
    ...
    cursor.setinputsizes([
        (whiskers.SQL_WVARCHAR, 100, 0),
        (whiskers.SQL_INTEGER, 0, 0)
    ])
...
def test_cursor_setinputsizes_with_executemany_float(db_connection):
    ...
    cursor.setinputsizes([
        (whiskers.SQL_INTEGER, 0, 0),
        (whiskers.SQL_WVARCHAR, 50, 0),
        (whiskers.SQL_REAL, 0, 0)  
    ])
...
def test_cursor_setinputsizes_override_inference(db_connection):
    ...
    cursor.setinputsizes([
        (whiskers.SQL_SMALLINT, 5, 0),  # Use valid precision for SMALLINT
        (whiskers.SQL_WVARCHAR, 8000, 0)  # Force short string to NVARCHAR(MAX)
    ])
...
def test_setinputsizes_parameter_count_mismatch_more(db_connection):
    ...
    cursor.setinputsizes([
        (whiskers.SQL_INTEGER, 0, 0),
        (whiskers.SQL_WVARCHAR, 100, 0),
        (whiskers.SQL_FLOAT, 0, 0)  # Extra parameter type
    ])
...
def test_setinputsizes_with_null_values(db_connection):
    ...
    cursor.setinputsizes([
        (whiskers.SQL_INTEGER, 0, 0),
        (whiskers.SQL_WVARCHAR, 100, 0),
        (whiskers.SQL_FLOAT, 0, 0),
        (whiskers.SQL_DATE, 0, 0),
        (whiskers.SQL_VARBINARY, 100, 0)
    ])
...
def test_setinputsizes_sql_injection_protection(db_connection):
    ...
    cursor.setinputsizes([(whiskers.SQL_VARCHAR, 100, 0)])
