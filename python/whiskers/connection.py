...
import threading
from whiskers.cursor import Cursor
from whiskers.helpers import add_driver_to_connection_str, sanitize_connection_string, sanitize_user_input, log
from whiskers import ddbc_bindings
from whiskers.pooling import PoolingManager
from whiskers.exceptions import InterfaceError, ProgrammingError
from whiskers.auth import process_connection_string
...
from whiskers.constants import GetInfoConstants
...
class Connection:
    ...
    def __init__(self, connection_str: str = "", autocommit: bool = False, attrs_before: dict = None, timeout: int = 0, **kwargs) -> None:
        ...
        try:
            ...
            pooled = ddbc_bindings._pool.get(self.connection_str) if self._pooling else None
    ...
    def setencoding(self, encoding=None, ctype=None):
        ...
        Example:
            ...
            cnxn.setencoding(encoding='utf-8', ctype=whiskers.SQL_CHAR)
        """
        if self._closed:
    ...
    def setdecoding(self, sqltype, encoding=None, ctype=None):
        ...
        Example:
            ...
            cnxn.setdecoding(whiskers.SQL_CHAR, encoding='utf-8')
            
            # Configure column metadata decoding
            cnxn.setdecoding(whiskers.SQL_WMETADATA, encoding='utf-16le')
            
            # Use explicit ctype
            cnxn.setdecoding(whiskers.SQL_WCHAR, encoding='utf-16le', ctype=whiskers.SQL_WCHAR)
        """
        if self._closed:
            raise InterfaceError(
                driver_error="Connection is closed",
    ...
    def getdecoding(self, sqltype):
        ...
        Example:
            settings = cnxn.getdecoding(whiskers.SQL_CHAR)
    ...
    def close(self) -> None:
        ...
        try:
            if self._conn:
                ...
                if self._pooling and ddbc_bindings._pool._enabled:
                    try:
                        # Test if connection is still valid before pooling
                        h = self._conn.alloc_statement_handle()
                        h.free()
                        ddbc_bindings._pool.put(self.connection_str, self._conn)
                        self._conn = None
                    except Exception:
                        # Connection is invalid, just close it
                        try:
                            self._conn.close()
                        except Exception:
                            pass
