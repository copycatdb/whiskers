...
from whiskers.constants import ConstantsDDBC as ddbc_sql_const, SQLTypes
from whiskers.helpers import check_error, log
from whiskers import ddbc_bindings
from whiskers.exceptions import InterfaceError, NotSupportedError, ProgrammingError
from whiskers.row import Row
from whiskers import get_settings
...
class Cursor:
    ...
    def __init__(self, connection, timeout: int = 0) -> None:
        ...
        self._last_fetch_size = 0  # Track size of last fetch block for skip adjustment
    ...
    def fetchmany(self, size: int = None) -> List[Row]:
        ...
        try:
            ...
            if rows_data and self._has_result_set:
                ...
                self._rownumber = self._next_row_index - 1
                self._last_fetch_size = len(rows_data)
    ...
    def scroll(self, value: int, mode: str = 'relative') -> None:
        ...
        try:
            if mode == 'relative':
                if value == 0:
                    return
                # Adjust for multi-row fetch blocks: _rownumber points to the last row
                # in the fetch block, but skip/scroll should be relative to the first row.
                adjustment = max(0, self._last_fetch_size - 1)
                effective_offset = value - adjustment
                if effective_offset <= 0:
                    effective_offset = value  # fallback if adjustment overshoots
                ret = ddbc_bindings.DDBCSQLFetchScroll(self.hstmt,
                                                       ddbc_sql_const.SQL_FETCH_RELATIVE.value,
                                                       effective_offset, row_data)
                if ret == ddbc_sql_const.SQL_NO_DATA.value:
                ...
                self._last_fetch_size = 0  # Reset after scroll
                return
